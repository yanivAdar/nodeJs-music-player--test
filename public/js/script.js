$(document).ready(() => {
    ...
    const initAppend = (dataObjArr) => {
        ...
        dataObjArr.forEach((x) => {
            g_AllPlaylistObjArray[x._id] = { id: x._id, name: x.name, image: x.image };
            ajaxRequest('get', '/' + x._id + '/songs', data => g_AllPlaylistObjArray[x._id].songs = data.songs);
            appending(x);
        })
        playJuckBox();
        removeEdit();
    ...
    const appending = (x) => {
        $('.main-playlist').append(`<div id="playlistDiv` + x._id + `" class="col-lg-3 col-md-3 col-sm-3 col-xs-3 single-playlist">
                                        <h2 value="`+ x.name + `" id="playlistTitle` + x._id + `" class="headLine">` + x.name + `</h2>
        ...
        $('#playlistDiv' + x._id).css('background-image', 'url(' + x.image + ')')
        let roundedText = new CircleType($('#playlistTitle' + x._id)[0]);
    ...
    const clickForNextModel = (id) => {
        ...
        if (id) {
            ajaxRequest('patch', '/' + id, (data) => init(), dataObj);
        }
        return dataObj;
    }
    ...
    const clickToFinish = (id, cb, dataObj) => {
        ...
        let songsObj = { songs: makeSongsArr($('.songUrl'), $('.songName')) };
        if (id) {
            return ajaxRequest('patch', '/' + id + '/songs', () => init(), songsObj);
        }
        Object.assign(dataObj, songsObj);
        ajaxRequest('put', '', (data) => {
            init();
